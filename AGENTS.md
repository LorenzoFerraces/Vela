...
- **Default to subagents for everything that is not coordination.** File reads, searches, investigations, test runs, and every task of a multi-task plan are dispatched to a subagent (implementer `general`, discovery `explore`) — one at a time, strictly sequential. The main session keeps only: the todo/ledger state, dispatch prompts, short status reports, and small self-contained edits (one-line rule or config changes). Anything that would fill the main context with multi-line output or file dumps goes into a file a subagent writes, referenced by path — never pasted into the main conversation.
