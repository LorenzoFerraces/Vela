...
Use a **single JSON file** for dynamic config (not a directory). Point `VELA_TRAEFIK_DYNAMIC_FILE` at it and set `VELA_TRAFFIC_ROUTER=traefik_file`. Put Traefik and app containers on the same Docker network (`VELA_DOCKER_NETWORK`). See [Traefik docs](https://doc.traefik.io/traefik/) for TLS and entrypoints.
